...
public class ProductService {
    ...
    public ProductDto getProductOfTheDay() {
        List<Product> discounted = repository.findByDiscountPriceIsNotNull().stream()
                .filter(p -> p.getDiscountPrice().compareTo(p.getPrice()) < 0)
                .toList();

        if (discounted.isEmpty()) {
            throw new ResourceNotFoundException("No discounted products found");
        }

        BigDecimal maxDiscount = discounted.stream()
                .map(p -> p.getPrice().subtract(p.getDiscountPrice()))
                .max(BigDecimal::compareTo)
                .orElseThrow();

        List<Product> maxDiscounted = discounted.stream()
                .filter(p -> p.getPrice().subtract(p.getDiscountPrice()).compareTo(maxDiscount) == 0)
                .toList();

        Product random = maxDiscounted.get(new Random().nextInt(maxDiscounted.size()));
        return mapper.entityToDto(random);
    }
}
